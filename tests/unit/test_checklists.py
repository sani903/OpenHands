...
from openhands.events.action import MessageAction, AgentFinishAction, AgentRejectAction
...
class TestRealPrePostConditions:
    ...
    def controller(self, event_stream, real_agent):
        ...
        controller = AgentController(
            ...
            postconditions_model=os.environ.get("TEST_LLM_MODEL", "neulab/claude-3-5-haiku-20241022"),
            preconditions_model=os.environ.get("TEST_LLM_MODEL", "neulab/claude-3-5-haiku-20241022"),
        )
    ...
    async def test_preconditions_real_generation(self, controller):
        ...
        user_message = MessageAction(content="Create a Python script that sorts a list of numbers.")
        ...
        assert controller.initial_task == "Create a Python script that sorts a list of numbers."
        
        # Check augmented message format
        assert "Generated Checklist" in user_message.content
        assert "Create a Python script that sorts a list of numbers." in user_message.content
    ...
    async def test_postconditions_real_generation(self, controller):
        ...
        controller.event_stream.add_event(
            MessageAction(content="Create a Python script that sorts a list of numbers."),
            EventSource.USER
        )
        controller.event_stream.add_event(
...
"""),
    ...
    async def test_multiple_iterations_stuck_detection(self, controller):
        ...
        controller.event_stream.add_event(
            MessageAction(content="I'll write a factorial function."),
            EventSource.AGENT
        )
        ...
        events = list(controller.event_stream.get_events())
        checklist_msg = [e for e in events if isinstance(e, MessageAction) and "CHECKLIST" in e.content]
    ...
    async def test_max_iterations_reached(self, controller):
        ...
        controller.event_stream.add_event(
            MessageAction(content="Here's a factorial function:"),
            EventSource.AGENT
        )
        ...
        events = list(controller.event_stream.get_events())
        max_iter_msg = [e for e in events if isinstance(e, MessageAction) and "maximum number of steps" in e.content]
    ...
    async def test_very_large_prompt(self, controller):
        ...
        large_content = "Task: Create a Python function. " + ("Details: This is part of the requirements. " * 100)
